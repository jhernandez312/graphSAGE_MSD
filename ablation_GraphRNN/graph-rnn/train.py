...
def train_graph_rnn(
    ...
    checkpoint_path = (
        ...
        else Path(base_path) / config['train']['checkpoint_dir']
    )
    log_path = (
        Path(log_dir)
        if log_dir is not None
        else Path(base_path) / config['train']['log_dir']
    )
    ...
    global_step = 0
    last_checkpoint = Path(restore_path).resolve() if restore_path else None

    # Restore from checkpoint
    if restore_path:
        print("Restoring from checkpoint: {}".format(restore_path))
        state = torch.load(restore_path, map_location=device, weights_only=False)
    ...
    while not done:
        for data in data_loader:
            ...
            if global_step % config['train']['checkpoint_iter'] == 0 or global_step+1 > config['train']['steps']:
                ...
                last_checkpoint = checkpoint_path / "checkpoint-{}.pth".format(global_step)
                torch.save(state, last_checkpoint)

    writer.close()
    if last_checkpoint is None or not last_checkpoint.exists():
        raise RuntimeError("GraphRNN training finished without producing a checkpoint.")
