...
class GraphDataSet(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset, m=None, bfs=True, training=True, train_split=0.8, graph_dir=None):
        """
        Arguments:
        ...
        if dataset == 'grid':
            ...
            self.graphs = self.load_swiss_dataset()
        else:
    ...
    def load_swiss_dataset(self, min_nodes=4, max_nodes=36):
        """
        Load Swiss dwelling graphs from pickle files.

        Keeps only graphs that:
        - are connected
        - have number of nodes in [min_nodes, max_nodes]

        Requires self.graph_dir to point to the folder containing .pickle files.
        """
        ...
        # discover pickle file in "dir" (specified in config_swiss.yaml)
        # sort files
        graph_files = sorted(
            [f for f in os.listdir(self.graph_dir) if f.endswith(".pickle")],
            key=lambda x: int(os.path.splitext(x)[0])
        )

        for f in graph_files:
            g = load_pickle(os.path.join(self.graph_dir, f))
            ...
            # IMPORTANT:
            # __getitem__ creates result with (num_nodes - 1) rows,
            # so max_node_count should track that sequence length
            self.max_node_count = max(self.max_node_count, num_nodes - 1)

        print(f"Kept {len(retval)} Swiss graphs after filtering.")
